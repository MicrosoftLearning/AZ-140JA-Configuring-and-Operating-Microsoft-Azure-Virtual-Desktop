...
1. **az140-cl-vm42** ブレードで、「**接続**」を選択し、ドロップダウン メニューで、「**Bastion**」を選択し、「**az140-cl-vm42 \| 接続**」ブレードの「Bastion」タブで、「**Bastion の使用**」を選択します。
1. プロンプトが表示されたら、**wvdadmin1** ユーザー名とこのユーザー アカウントの作成時に設定したパスワードを使用してサインインします。 
...
1. Hyper-V コンポーネントのインストールが完了したら、**Y** と入力し、**Enter** キーを押してオペレーティング システムを再起動します。再起動後、パスワード **PA55w.rd1234** で **wvdadmin1** アカウントを使用して再度サインインします。
