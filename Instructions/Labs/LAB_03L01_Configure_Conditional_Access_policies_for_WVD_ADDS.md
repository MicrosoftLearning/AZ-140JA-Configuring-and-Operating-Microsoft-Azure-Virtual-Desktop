...
#### タスク 1: Azure AD Premium ライセンスの構成する
...
#### タスク 2: Azure AD Multi-Factor Authentication (MFA) の構成
...
1. 「**ID 保護 | 概要」** ブレードの左側の垂直メニューの **「保護」** セクションで、**「MFA 登録ポリシー」** をクリックします。
...
1. **「Microsoft Authenticator」** ページで、**「別の方法を設定する」** リンクをクリックします。
1. **「別の方法を選択」** ダイアログ ボックスで、**「どの方法を使用しますか?」** ドロップダウン リストで、**「電話」** を選択し、**「確認」** をクリックします。
1. **「電話」** ページで、電話番号を入力し、**「コードをテキスト メッセージで送信」** オプションが選択されていることを確認して、**「次へ」** をクリックします。
1. **「電話」** ページで、電話で受信したテキストに含まれているコードを入力し、**「次へ」** をクリックします。
1. SMSが正常に検証され、電話が正常に登録されたことを確認して、**「次へ」** をクリックします。
1. **「成功!」** ページで、**「完了」** をクリックします。
1. Azure portal ページの右上隅にあるユーザー アバターを表すアイコンをクリックし、**「サインアウト」** をクリックして、**In private** ブラウザーの画面を閉じます。 

#### タスク 4: ハイブリッド Azure AD 参加の構成
...
>**注**: このタスクでは、MFA が Windows Virtual Desktop セッションにサインインすることを要求する Azure AD ベースの条件付きアクセスポリシーを構成します。ポリシーは、認証が成功してから最初の 4 時間後に再認証も実施します。
